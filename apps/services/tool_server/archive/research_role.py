...
DEPRECATED (2026-01-27): This module is no longer used.

internet_research_mcp.py now calls apps/tools/internet_research/ directly,
bypassing this orchestration layer. The LLM-driven research API handles
strategy selection internally.
...
# Import dependencies
from apps.services.tool_server import research_orchestrator
from apps.services.tool_server.session_intelligence_cache import get_intelligence_cache
from apps.services.tool_server.research_evaluator import evaluate_research_satisfaction
...
class ResearchRole:
    ...
    async def orchestrate(
        ...
        # Trigger site_search intent to navigate directly to the site and find the content
        ...
        if content_reference:
            ...
            # CASE 1: We have the actual URL - navigate directly to it
            if source_url:
                intent = "navigation"
                intent_metadata = {
                    "target_url": source_url,
                    "goal": f"Read content: {title[:50] if title else 'content'}"
                }
                logger.info(
                    f"[ResearchRole] Content reference has URL - using direct navigation: {source_url[:60]}..."
                )
                # Use a query that describes what we're reading
                query = f"Read {title[:50] if title else 'content'} from {site or 'web'}"

                # Update research_context with navigation intent
                research_context["intent"] = intent
                research_context["intent_metadata"] = intent_metadata

            # CASE 2: No URL but have site + title - use site_search to find the content
            # This triggers the SITE_SEARCH path in research_orchestrator which:
            # 1. Navigates directly to the site
            # 2. Extracts all links from the page
            # 3. Searches for matching thread/article by title
            # 4. Clicks through to the matching content
            #
            # IMPORTANT: Do NOT override intent if it's 'commerce' - for commerce queries,
            # the content_reference is just a hint about a previous source, not a directive
            # to ONLY search that source. Commerce queries need multi-vendor Phase 2 search.
            elif site and title:
                ...
                # Only use site_search for non-commerce intents
                # Commerce queries should still do full multi-vendor search in Phase 2
                if intent != "commerce":
                    # Override intent to site_search to trigger direct navigation path
                    intent = "site_search"
                    intent_metadata = {
                        "site_name": site_normalized,
                        "search_term": title
                    }
                    logger.info(
                        f"[ResearchRole] Content reference triggering site_search: "
                        f"site={site_normalized}, search_term='{title[:50]}...'"
                    )
                    # Keep query empty so SITE_SEARCH path uses direct navigation
                    query = ""

                    # CRITICAL: Update research_context with modified intent so it flows to gather_intelligence
                    research_context["intent"] = intent
                    research_context["intent_metadata"] = intent_metadata
                else:
                    # Commerce intent: keep commerce intent, but log the content reference
                    # The reference site will be visited as part of normal Phase 2 vendor search
                    logger.info(
                        f"[ResearchRole] Content reference for commerce query (NOT overriding intent): "
                        f"site={site_normalized} - will be included in multi-vendor search"
                    )
        ...
        try:
            from apps.services.tool_server.knowledge_extractor import get_knowledge_extractor
    ...
    async def _generate_requirements_reasoning(
        ...
        # Load prompt template from recipe system
        try:
            recipe = load_recipe("research/requirements_reasoning")
            prompt_template = recipe.get_prompt()
        except RecipeNotFoundError:
            logger.error("[ResearchRole] Requirements reasoning recipe not found")
            return {
                "reasoning_document": "",
                "parsed": {},
                "optimized_query": query
...
- Appropriate search terms"""
    ...
    async def _generate_research_goals(
        ...
        try:
            recipe = load_recipe("research/goal_generator")
            base_prompt = recipe.get_prompt()
        except RecipeNotFoundError:
            logger.warning("[ResearchRole] goal_generator recipe not found, using fallback prompt")
...
Generate goals now. Return ONLY JSON:"""
    ...
    async def _summarize_page_for_goal(
        ...
        try:
            ...
            logger.warning("[ResearchRole] page_summarizer recipe not found, using fallback prompt")
...
Provide your summary:"""
    ...
    async def _index_phase1_research(
        ...
        try:
            ...
            from libs.gateway.research.research_doc_writers import normalize_topic, generate_topic_hash
            import hashlib
