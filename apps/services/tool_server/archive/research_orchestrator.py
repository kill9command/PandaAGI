...
DEPRECATED (2026-01-27): Use apps/tools/internet_research/ instead.

This module is NO LONGER USED. The call chain has been simplified:
    OLD: internet_research_mcp → research_role → research_orchestrator
    NEW: internet_research_mcp → apps/tools/internet_research

The new LLM-driven API provides:
- Simple search()/visit()/done() tools
- LLM decides all strategy (no hardcoded routing)
- Document-based state management
- ~2,000 lines vs 7,769 lines here

Migration:
    OLD: gather_intelligence(query, ...), research(query, ...)
    NEW: from apps.tools.internet_research import execute_full_research
         await execute_full_research(goal=query, intent="commerce")

This file is kept for reference only. Do not add new features here.

Created: 2025-11-15
Deprecated: 2026-01-27 (replaced by apps/tools/internet_research/)
"""
...
# Recipe system for prompt loading
from libs.gateway.llm.recipe_loader import load_recipe, RecipeNotFoundError

# NEW: Two-phase research system imports
from apps.services.tool_server.query_planner import optimize_query_for_phase, construct_vendor_search_url, simplify_query_for_retailers, simplify_query_for_retailers_async, detect_price_priority
from apps.services.tool_server.llm_candidate_filter import filter_candidates_for_intelligence
from apps.services.tool_server.shared_state.source_reliability import get_tracker
from apps.services.tool_server.retailer_selector import select_retailers_for_query
from apps.services.tool_server.product_comparison_tracker import ProductComparisonTracker

from apps.services.tool_server.product_requirements import ProductRequirements

# Universal Web Agent - simple See→Think→Act loop
from apps.services.tool_server.web_agent import WebAgent, WebAgentResult, get_web_agent
...
async def _goal_directed_browse(
    ...
    """
    Use WebAgent for goal-directed browsing.
    ...
    try:
        ...
        # Create WebAgent with site knowledge
        agent = WebAgent(
            page=page,
        ...
        # Convert to result dict format for compatibility
        result = {
            "pages_visited": [url],
            "goal_achieved": len(products) > 0,
...
def _build_navigation_context(
    ...
    """
    Build navigation context for WebAgent from Phase 1 intelligence.
...
async def _try_recovery_strategy(
    ...
    try:
        if strategy == "recalibrate_selectors":
            ...
            from apps.services.tool_server.shared_state.site_schema_registry import get_schema_registry
            ...
            from apps.services.tool_server.page_intelligence.legacy_adapter import get_calibrator
...
def quick_relevance_prefilter(
    products: List[Dict[str, Any]],
    query: str,
    requirements_reasoning: Optional[str] = None
) -> Dict[str, Any]:
    """
    Fast keyword-based pre-filter to remove obviously irrelevant products.

    Runs BEFORE the expensive LLM viability check to reduce the number
    of products that need LLM evaluation.

    Uses the LLM-generated disqualifiers from requirements_reasoning to identify
    wrong categories. This is NOT hardcoded - the LLM decides what's irrelevant
    for each specific query (e.g., "cage", "toy" for pet queries, or "bag", "stand"
    for laptop queries).

    Args:
        products: List of extracted products
        query: User's original query
        requirements_reasoning: YAML from requirements reasoning (has disqualifiers.wrong_category)

    Returns:
        {
            "filtered_products": List of products that passed relevance check,
            "removed_count": Number of products removed,
            "removed_products": List of removed products with reasons
        }
    """
    import yaml
    import re
    ...
    if not products:
        return {"filtered_products": [], "removed_count": 0, "removed_products": []}

    # Parse requirements reasoning to get LLM-generated disqualifiers
    core_product = None
    wrong_categories = []
    avoid_terms = []
    ...
    if requirements_reasoning:
        try:
            ...
            if isinstance(parsed, dict):
                # Get core product from query_understanding
                query_understanding = parsed.get("query_understanding", {})
                core_product = query_understanding.get("core_product", "")

                # Get LLM-identified wrong categories (THE KEY PART - not hardcoded)
                disqualifiers = parsed.get("disqualifiers", {})
                wrong_categories = disqualifiers.get("wrong_category", [])
                if isinstance(wrong_categories, str):
                    wrong_categories = [wrong_categories]

                # Get search optimization avoid terms
                search_opt = parsed.get("search_optimization", {})
                avoid_terms = search_opt.get("avoid_terms", [])
                if isinstance(avoid_terms, str):
                    avoid_terms = [avoid_terms]

        except Exception as e:
            logger.debug(f"[QuickRelevanceFilter] Could not parse requirements reasoning: {e}")

    # If no LLM-generated disqualifiers, skip pre-filtering
    # Let the full LLM viability check handle it - don't use hardcoded rules
    if not wrong_categories and not avoid_terms:
        logger.debug("[QuickRelevanceFilter] No disqualifiers from requirements reasoning, skipping pre-filter")
        return {"filtered_products": products, "removed_count": 0, "removed_products": []}

    # Fall back to extracting core product from query if not in reasoning
    if not core_product:
        core_product = query.lower()
        for word in ["buy", "for sale", "online", "cheapest", "best", "find", "where", "shop", "purchase", "sale"]:
            core_product = core_product.replace(word, "")
        core_product = " ".join(core_product.split())

    core_product_lower = core_product.lower().strip()

    # Extract key product words FIRST (before parsing disqualifiers)
    key_words = [w for w in core_product_lower.split() if len(w) > 2]
    key_words_expanded = set(key_words)
    for word in key_words:
        if word.endswith("s"):
            key_words_expanded.add(word[:-1])
        else:
            key_words_expanded.add(word + "s")

    # Combine wrong categories and avoid terms into disqualifier PHRASES
    # IMPORTANT: Keep phrases intact instead of splitting into words
    # e.g., "stuffed animal" should match "stuffed animal toy" but NOT "live animal"
    #
    # NO SAFELIST - LLM decisions should not be overridden with hardcoded lists
    # If LLM is generating bad disqualifiers, fix the prompt, not the code
    disqualifier_phrases = set()
    for term in wrong_categories + avoid_terms:
        # Normalize: remove parentheticals, normalize whitespace
        # "plush toy (fake)" -> "plush toy"
        # "stuffed animal" -> "stuffed animal" (kept as phrase!)
        cleaned = re.sub(r'\([^)]*\)', '', term.lower())  # Remove parentheticals
        cleaned = re.sub(r'[^a-z\s]', ' ', cleaned)  # Remove non-alpha
        cleaned = ' '.join(cleaned.split())  # Normalize whitespace

        # Skip if the phrase IS the product (e.g., don't disqualify "hamster" for hamster query)
        if cleaned and len(cleaned) > 2:
            # Check if phrase overlaps significantly with key product words
            phrase_words = set(cleaned.split())
            overlap = phrase_words & key_words_expanded
            # Only add if less than half the phrase is key words
            # e.g., "hamster book" for hamster query -> "hamster" overlaps, but "book" doesn't
            # We want to KEEP "hamster book" as disqualifier
            if len(overlap) < len(phrase_words):
                disqualifier_phrases.add(cleaned)
    ...
    for product in products:
        ...
        # Normalize product name for phrase matching
        name_normalized = ' '.join(name.split())

        # Check if product name contains at least one key word from the query
        has_key_word = any(kw in name for kw in key_words_expanded)

        # Check if product name contains LLM-identified disqualifier PHRASES
        # e.g., "stuffed animal" matches "cute stuffed animal toy" but NOT "live animal for sale"
        matching_disqualifiers = [phrase for phrase in disqualifier_phrases if phrase in name_normalized]

        # Decision logic:
        # - If it has no key words at all -> remove
        # - If it matches LLM-identified disqualifiers -> remove
        # - Otherwise -> keep for LLM evaluation

        if not has_key_word:
            removed.append({
                **product,
                "prefilter_reason": f"No key words from query ({', '.join(list(key_words)[:3])}) in product name"
            })
        elif matching_disqualifiers:
            removed.append({
                **product,
                "prefilter_reason": f"Matches disqualifier: {', '.join(matching_disqualifiers[:2])}"
            })
        else:
    ...
    if removed:
        logger.info(
            f"[QuickRelevanceFilter] Removed {len(removed)}/{len(products)} products using LLM disqualifiers. "
            f"Phrases: {list(disqualifier_phrases)[:5]}"
        )
        for r in removed[:3]:
            logger.debug(f"[QuickRelevanceFilter] Removed: {r.get('name', '')[:50]} - {r.get('prefilter_reason', '')}")
...
try:
    from apps.services.tool_server.document_tools import get_document_manager
except ImportError:
...
try:
    from apps.services.tool_server.browser_agent import deep_browse, detect_navigation_opportunities
except ImportError:
...
async def gather_intelligence(
    ...
    """
    from apps.services.tool_server import web_vision_mcp
    from apps.services.tool_server.llm_candidate_filter import llm_filter_candidates
    ...
    from apps.services.tool_server.query_planner import plan_phase1_queries
    ...
    prior_turn_context = research_context.get("prior_turn_context")
    ...
    # For commerce sites (Amazon, BestBuy, etc.), use WebAgent to search and extract products
    # For forum/content sites, use link-matching to find relevant threads
        ...
        if site_name:
            ...
            try:
                ...
                # First, navigate to the site to determine its type
                ...
                if not nav_result.get("success"):
                    logger.warning(f"[Phase1] SITE_SEARCH: Navigation failed to {target_url}")
                else:
                    # Get page understanding to determine if this is a commerce site
                    from apps.services.tool_server.page_intelligence.service import get_page_intelligence_service
                    page_intelligence = get_page_intelligence_service()
                    page = await web_vision_mcp.get_page(web_vision_session_id)

                    if page:
                        understanding = await page_intelligence.understand_page(
                            page=page,
                            extraction_goal=f"Search for: {search_term}"
                        )

                        # Get page type as string for comparison
                        page_type_str = str(understanding.page_type.value) if understanding and understanding.page_type else ""
                        zones = [str(z.zone_type) if hasattr(z.zone_type, 'value') else str(z.zone_type) for z in understanding.zones] if understanding and understanding.zones else []

                        logger.info(f"[Phase1] SITE_SEARCH: Page type={page_type_str}, zones={zones}")

                        # Detect commerce site: homepage with search controls or product grid
                        # Commerce indicators: search_controls zone, product_grid zone, homepage with shopping features
                        # PageType values: homepage, search_results, product_detail, category, list_page
                        is_commerce_site = (
                            page_type_str in ["homepage", "search_results", "category", "product_detail", "list_page"] or
                            any("search" in z.lower() for z in zones) or
                            any("product" in z.lower() for z in zones) or
                            any(kw in domain for kw in ["amazon", "ebay", "walmart", "bestbuy", "newegg", "target", "costco"])
                        )

                        if is_commerce_site:
                            # COMMERCE SITE: Use WebAgent to search and extract products
                            logger.info(f"[Phase1] SITE_SEARCH: Detected commerce site, using WebAgent to search")
                            ...
                            # WebAgent will find search box, type query, and extract products
                            agent_result = await agent.navigate(
                                url=target_url,  # Already navigated, WebAgent will use current page
                                goal=f"Search for '{search_term}' and extract product listings with prices",
                                original_query=search_term,
                                max_steps=5
                            )

                            if agent_result and agent_result.products:
                                logger.info(f"[Phase1] SITE_SEARCH: WebAgent found {len(agent_result.products)} products")
                                # Convert products to context findings format
                                product_summaries = []
                                for p in agent_result.products[:10]:  # Limit to 10
                                    product_summaries.append(f"- {p.name}: {p.price} ({p.url})")

                                context_findings.append({
                                    "url": target_url,
                                    "title": f"Search results for '{search_term}' on {domain}",
                                    "summary": f"Found {len(agent_result.products)} products matching '{search_term}'",
                                    "text_content": "\n".join(product_summaries),
                                    "key_points": [f"Found {len(agent_result.products)} products"],
                                    "claims": [],
                                    "source_type": "commerce_search",
                                    "products": [p.to_dict() for p in agent_result.products],
                                    "extracted_links": []
                                })
                            else:
                                # WebAgent didn't find products - log determination
                                determination = agent_result.determination if agent_result else "error"
                                reason = agent_result.reason if agent_result else "WebAgent returned no result"
                                logger.info(f"[Phase1] SITE_SEARCH: WebAgent result: {determination} - {reason}")

                                context_findings.append({
                                    "url": target_url,
                                    "title": f"No products found on {domain}",
                                    "summary": f"Search for '{search_term}' returned no matching products. Reason: {reason}",
                                    "key_points": [f"No products found: {determination}"],
                                    "claims": [],
                                    "source_type": "commerce_search",
                                    "extracted_links": []
                                })
                        else:
                            # FORUM/CONTENT SITE: Use link-matching to find relevant content
                            logger.info(f"[Phase1] SITE_SEARCH: Content/forum site, using link matching")

                            site_search_reading_goal = f"Find thread or topic: {search_term}"
                            result = await _web_vision_visit_and_read(
                                url=target_url,
                                reading_goal=site_search_reading_goal,
                                session_id=web_vision_session_id
                            )
                            if result:
                                ...
                                for link in extracted_links:
                                    ...
                                    if matches >= len(search_words) * 0.6:
                                        matching_link = link
                        ...
                        logger.warning(f"[Phase1] SITE_SEARCH: Could not get page object for {target_url}")
            except Exception as e:
                logger.error(f"[Phase1] SITE_SEARCH: Error navigating to {target_url}: {e}")
                import traceback
                logger.error(f"[Phase1] SITE_SEARCH: Traceback: {traceback.format_exc()}")
        else:
...
async def search_products_with_comparison(
    ...
    # WebAgent is now the unified extraction system
    ...
    for retailer in retailers:
        ...
        # Visit and extract products using WebAgent (single unified system)
        try:
            from apps.services.tool_server import web_vision_mcp
            ...
            # ==========================================================================
            # EXTRACTION: WebAgent is the SINGLE unified system (no fallbacks)
            # Per WEB_AGENT_ARCHITECTURE.md - replaces perception_pipeline and HTML extraction
            # ==========================================================================
            products = []
            ...
            if use_hybrid_extraction:
                ...
                logger.info(f"[Phase2-Comparison] Using WebAgent for {retailer}")
                ...
                try:
                    ...
                    logger.warning(f"[Phase2-Comparison] WebAgent timed out for {retailer}")
                    continue

                logger.info(
                    f"[Phase2-Comparison] WebAgent result for {retailer}: "
                    f"determination={result.determination}, products={len(result.products)}"
                )

                if result.determination == "products_found":
                    for p in result.products:
                        product = {
                            "name": p.name,
                            "price": p.price if p.price else "N/A",
                            "url": p.url,
                            "description": p.description,
                            "confidence": p.confidence,
                            "vendor": retailer,
                            "extraction_method": "web_agent",
                            "pdp_verified": False,
                            "in_stock": p.in_stock,
                        }
                        products.append(product)
                        logger.info(f"[Phase2-Comparison] ✓ Found '{p.name[:40]}' price={p.price} url={p.url[:60]}...")

                elif result.determination == "no_relevant_products":
                    logger.info(f"[Phase2-Comparison] {retailer} has no relevant products: {result.reason}")
                    continue

                else:
                    logger.warning(f"[Phase2-Comparison] WebAgent failed for {retailer}: {result.reason}")
                    continue

            else:
                # LEGACY: HTML-only extraction (only used when PERCEPTION_ENABLE_HYBRID=false)
                from apps.services.tool_server.product_extractor import extract_products_from_html
...
async def intelligent_vendor_search(
    ...
    """
    from apps.services.tool_server import web_vision_mcp
    from apps.services.tool_server.llm_candidate_filter import select_vendor_candidates
    from apps.services.tool_server.product_viability import filter_viable_products, filter_viable_products_with_reasoning
    from apps.services.tool_server.product_comparison_tracker import ProductComparisonTracker
    ...
    if user_query:
        ...
        site_specific_queries = smart_queries.get("site_specific", {})
        # Ensure site_specific_queries is a dict (LLM may return a list)
        if not isinstance(site_specific_queries, dict):
            site_specific_queries = {}
    ...
    from apps.services.tool_server.query_planner import construct_vendor_search_url
    registry = get_vendor_registry()
    price_range = (intelligence.get("price_range") if intelligence else None) or {}
    ...
    phase1_vendors = []
    retailers_from_intel = (intelligence.get("retailers") if intelligence else None) or {}

    # Handle case where retailers is a list instead of dict
    if isinstance(retailers_from_intel, list):
        logger.info(f"[IntelligentSearch] Converting retailers list to dict: {retailers_from_intel}")
        retailers_from_intel = {
            name.lower() if isinstance(name, str) else str(name): {
                "relevance_score": 0.7,
                "context": "From intelligence list",
                "mentioned_for": [],
                "include_in_search": True
            }
            for name in retailers_from_intel
            if name
        }
    ...
    for vendor in vendor_candidates:
        ...
        try:
            ...
            # ==========================================================================
            # EXTRACTION: WebAgent is the SINGLE unified system (no fallbacks)
            # Per WEB_AGENT_ARCHITECTURE.md: "No fallbacks. If WebAgent fails to
            # extract products, it creates an intervention."
            # ==========================================================================
            raw_products = []
            webagent_determination = None  # Track WHY we got the result

            if use_hybrid_extraction:
                try:
                    page = await web_vision_mcp.get_page(web_vision_session_id)
                    if page:
                        logger.info(f"[IntelligentSearch] Using WebAgent for {vendor_domain}")
                        ...
                        # Create WebAgent with site knowledge cache
                        agent = WebAgent(
                            page=page,
                            knowledge_cache=get_site_knowledge_cache(),
                            llm_url=llm_url,
                        ...
                        # Extract using PERCEIVE→DECIDE→ACT→VERIFY loop
                        VENDOR_TIMEOUT = 90
                        try:
                            ...
                            logger.warning(f"[IntelligentSearch] WebAgent timed out after {VENDOR_TIMEOUT}s for {vendor_domain}")
                            # Build result from partial results
                            result = WebAgentResult(
                                products=agent.partial_results if hasattr(agent, 'partial_results') else [],
                                determination="error",
                                reason=f"Timeout after {VENDOR_TIMEOUT}s",
                                page_type=None,
                                items_seen=0
                            )

                        # Store determination for logging/debugging
                        webagent_determination = result.determination
                        logger.info(
                            f"[IntelligentSearch] WebAgent result for {vendor_domain}: "
                            f"determination={result.determination}, products={len(result.products)}, "
                            f"items_seen={result.items_seen}, reason={result.reason}"
                        )

                        # Handle determination - NO FALLBACKS
                        if result.determination == "products_found":
                            # SUCCESS: Convert Product objects to dict format
                            ...
                            logger.info(f"[IntelligentSearch] ✓ WebAgent extracted {len(raw_products)} products from {vendor_domain}")

                            # Record success
                            try:
                                from apps.services.tool_server.shared_state.site_health_tracker import get_health_tracker
                                ht = get_health_tracker()
                                ht.record_extraction(url=vendor_url, page_type="listing", success=True, method="web_agent")
                            except Exception as e:
                                logger.debug(f"[IntelligentSearch] Failed to record extraction success: {e}")

                        elif result.determination == "no_relevant_products":
                            # VALID RESULT: Page examined, nothing matches query
                            # This is NOT a failure - do NOT trigger fallbacks
                            logger.info(
                                f"[IntelligentSearch] WebAgent found no relevant products on {vendor_domain}: {result.reason}. "
                                f"(saw {result.items_seen} items, none matching)"
                            )
                            vendors_failed.append(vendor_domain)
                            # Skip to next vendor - no fallback extraction
                            continue

                        elif result.determination == "blocked":
                            # BLOCKED: Create intervention
                            logger.warning(f"[IntelligentSearch] {vendor_domain} is blocked: {result.reason}")
                            vendors_failed.append(vendor_domain)

                            try:
                                from apps.services.tool_server.captcha_intervention import request_intervention
                                ...
                                intervention = await request_intervention(
                                    ...
                                    session_id=web_vision_session_id,
                                    blocker_details={"vendor": vendor_domain, "reason": result.reason},
                                    cdp_url=novnc_url
                                )
                                logger.warning(f"[IntelligentSearch] Waiting for intervention: {intervention.intervention_id}")
                                await intervention.wait_for_resolution(timeout=120)
                            except Exception as intervention_err:
                                logger.debug(f"[IntelligentSearch] Intervention failed: {intervention_err}")
                            continue

                        elif result.determination in ["error", "wrong_page_type"]:
                            # ERROR: Record failure, create intervention if needed
                            logger.warning(f"[IntelligentSearch] WebAgent error on {vendor_domain}: {result.reason}")
                            vendors_failed.append(vendor_domain)

                            # Record failure for site health tracking
                            try:
                                from apps.services.tool_server.shared_state.site_health_tracker import get_health_tracker
                                ht = get_health_tracker()
                                ht.record_extraction(url=vendor_url, page_type="listing", success=False, method="web_agent")
                            except Exception as e:
                                logger.debug(f"[IntelligentSearch] Failed to record extraction failure: {e}")

                            # Create intervention for visibility
                            try:
                                from apps.services.tool_server.captcha_intervention import request_intervention
                                novnc_url = "http://localhost:6080/vnc_lite.html?host=localhost&port=6080&scale=true"
                                intervention = await request_intervention(
                                    ...
                                    screenshot_path=None,
                                    session_id=web_vision_session_id,
                                    blocker_details={"vendor": vendor_domain, "query": query, "reason": result.reason},
                                    cdp_url=novnc_url
                                )
                                logger.warning(f"[IntelligentSearch] ❌ Extraction failed for {vendor_domain}. Intervention: {intervention.intervention_id}")
                                await intervention.wait_for_resolution(timeout=60)
                            except Exception as intervention_err:
                                logger.debug(f"[IntelligentSearch] Intervention failed: {intervention_err}")
                            continue

                except Exception as extract_err:
                    logger.warning(f"[IntelligentSearch] WebAgent exception for {vendor_domain}: {extract_err}")
                    vendors_failed.append(vendor_domain)
                    continue

            # If we reach here without products and didn't use hybrid extraction, skip vendor
            if not raw_products:
                logger.warning(f"[IntelligentSearch] No products from {vendor_domain}, moving to next vendor")
                if vendor_domain not in vendors_failed:
                    vendors_failed.append(vendor_domain)
                continue
            ...
            # ==================== STEP 4.6: Quick Relevance Pre-Filter ====================
            # Fast keyword-based filter to remove obviously irrelevant products (e.g., accessories)
            # before the expensive LLM viability check. Example: filter "hamster cage" when
            # looking for live hamsters.
            if raw_products and len(raw_products) > 10:
                reasoning_doc = requirements_reasoning.get("reasoning_document", "") if requirements_reasoning else ""
                prefilter_result = quick_relevance_prefilter(
                    products=raw_products,
                    query=query,
                    requirements_reasoning=reasoning_doc
                )
                if prefilter_result["removed_count"] > 0:
                    logger.info(
                        f"[IntelligentSearch] {vendor_domain}: Quick relevance filter removed "
                        f"{prefilter_result['removed_count']}/{len(raw_products)} irrelevant products"
                    )
                    raw_products = prefilter_result["filtered_products"]

                    if not raw_products:
                        logger.warning(f"[IntelligentSearch] {vendor_domain}: No products after relevance filter")
                        continue
        ...
        if fallback_vendors:
            ...
            for vendor in fallback_vendors:
                ...
                try:
                    ...
                    # Use WebAgent (single unified system - no fallbacks)
                    raw_products = []
                    ...
                    if use_hybrid_extraction:
                        try:
                            ...
                            if page:
                                ...
                                logger.info(
                                    f"[IntelligentSearch] FALLBACK: WebAgent result for {vendor_domain}: "
                                    f"determination={result.determination}, products={len(result.products)}"
                                )
                                ...
                                if result.determination == "products_found":
                                    for product in result.products:
                                        raw_products.append({
                                            ...
                                            "pdp_verified": False,
                                            "in_stock": product.in_stock if product.in_stock is not None else True,
                                            "extraction_method": "fallback_web_agent",
                                            "confidence": product.confidence
                                        })
                                    try:
                                        from apps.services.tool_server.shared_state.site_health_tracker import get_health_tracker
                                        ht = get_health_tracker()
                                        ht.record_extraction(url=vendor_url, page_type="listing", success=True, method="fallback_web_agent")
                                    except Exception as e:
                                        logger.debug(f"[IntelligentSearch] FALLBACK: Failed to record success: {e}")

                                elif result.determination == "no_relevant_products":
                                    # Valid result - no matching products, skip to next
                                    logger.info(f"[IntelligentSearch] FALLBACK: {vendor_domain} has no relevant products: {result.reason}")
                                    vendors_failed.append(vendor_domain)
                                    continue

                                else:
                                    # Error/blocked - record failure
                                    logger.warning(f"[IntelligentSearch] FALLBACK: {vendor_domain} failed: {result.reason}")
                                    vendors_failed.append(vendor_domain)
                                    continue

                        except asyncio.TimeoutError:
                            logger.warning(f"[IntelligentSearch] FALLBACK: WebAgent timed out for {vendor_domain}")
                            vendors_failed.append(vendor_domain)
                            continue
                        except Exception as extract_err:
    ...
    # ==================== STEP 6b: Retry Search if Short of Min Vendors ====================
    # If we have SOME vendors but not enough, do another search to find more
    if 0 < vendors_with_products < min_vendors:
        vendors_needed = min_vendors - vendors_with_products
        logger.info(
            f"[IntelligentSearch] RETRY SEARCH: Have {vendors_with_products}/{min_vendors} vendors, "
            f"searching for {vendors_needed} more..."
        )

        # Build a retry query - add "buy" or "store" to find more vendors
        retry_query = f"{shopping_query} where to buy"

        try:
            # Search for more vendors
            retry_results = await _web_vision_search(
                query=retry_query,
                max_results=10,
                session_id=web_vision_session_id,
                event_emitter=event_emitter
            ...
            if retry_results:
                ...
                already_visited = set(vendors_visited + vendors_failed)

                # Filter and select new vendors
                from apps.services.tool_server.llm_candidate_filter import select_vendor_candidates
                retry_candidates = await select_vendor_candidates(
                    search_results=retry_results,
                    query=retry_query,
                    intelligence=intelligence,
                    max_vendors=vendors_needed + 2,  # Get extras in case some fail
                    original_query=original_query
                )

                # Filter out already-visited vendors
                new_vendors = []
                for v in (retry_candidates or []):
                    domain = v.get("domain", "").lower().replace("www.", "")
                    if domain and domain not in already_visited:
                        new_vendors.append(v)
                        if len(new_vendors) >= vendors_needed + 2:
                            break

                logger.info(f"[IntelligentSearch] RETRY: Found {len(new_vendors)} new vendor candidates")

                # Visit the new vendors
                for vendor in new_vendors:
                    if vendors_with_products >= min_vendors:
                        logger.info(f"[IntelligentSearch] RETRY: Reached min_vendors ({min_vendors}), stopping")
                        break
                    ...
                    logger.info(f"[IntelligentSearch] RETRY: Visiting {vendor_domain}")
                    ...
                    try:
                        ...
                        if not nav_result.get("success"):
                            logger.warning(f"[IntelligentSearch] RETRY: Failed to navigate to {vendor_domain}")
                            vendors_failed.append(vendor_domain)
                            continue

                        # Use WebAgent for extraction
                        page = await web_vision_mcp.get_page(web_vision_session_id)
                        if page:
                            llm_url = os.getenv("SOLVER_URL", "http://127.0.0.1:8000/v1/chat/completions")
                            llm_model = os.getenv("SOLVER_MODEL_ID", "qwen3-coder")
                            ...
                            logger.info(
                                f"[IntelligentSearch] RETRY: WebAgent result for {vendor_domain}: "
                                f"determination={result.determination}, products={len(result.products)}"
                            )

                            if result.determination == "products_found" and result.products:
                                vendors_visited.append(vendor_domain)
                                vendors_with_products += 1

                                # Process products through viability filter
                                for product in result.products:
                                    product_dict = {
                                        "name": product.name,
                                        "price": product.price if product.price else "N/A",
                                    ...
                                    tracker.add_product(product_dict, vendor_domain)

                                logger.info(f"[IntelligentSearch] RETRY: ✓ Got {len(result.products)} products from {vendor_domain}")
                            else:
                                vendors_failed.append(vendor_domain)
                        else:
                            vendors_failed.append(vendor_domain)
                        ...
                        logger.error(f"[IntelligentSearch] RETRY: Error with {vendor_domain}: {retry_err}")
                        vendors_failed.append(vendor_domain)

        except Exception as retry_search_err:
            logger.warning(f"[IntelligentSearch] RETRY SEARCH failed: {retry_search_err}")
    ...
    if not min_vendors_met:
        ...
        try:
            from apps.services.tool_server.intervention_manager import log_extraction_failed
...
async def _build_smart_shopping_queries(
    ...
    # Extract relevant intelligence (use 'or' to handle None values)
    specs = (intelligence.get("specs_discovered") if intelligence else None) or {}
    hard_reqs = (intelligence.get("hard_requirements") if intelligence else None) or []
    price_range = (intelligence.get("price_range") if intelligence else None) or {}
    retailers = (intelligence.get("retailers") if intelligence else None) or {}
    ...
    if retailers:
        if isinstance(retailers, dict):
            ...
            # If retailers is a list of names, use them all
            high_relevance_retailers = retailers[:5]
    ...
    try:
        recipe = load_recipe("research/shopping_query_generator")
        base_prompt = recipe.get_prompt()
    except RecipeNotFoundError:
        logger.warning("[SmartQuery] Recipe not found, using fallback prompt")
...
Generate the optimized queries now. Return JSON ONLY:"""
    ...
    try:
        ...
        if result and isinstance(result, dict):
            ...
            # Ensure site_specific is a dict (LLM may return a list)
            if not isinstance(site_specific, dict):
                site_specific = {}
    ...
    from apps.services.tool_server.shared.spec_utils import get_spec_value
    spec_parts = []
    if specs:
    ...
    for search_query in queries:
        ...
        for candidate in candidates:
            ...
            try:
                ...
                # ==========================================================================
                # EXTRACTION: WebAgent is the SINGLE unified system (no fallbacks)
                # Phase 2 is product search - ALL URLs use WebAgent
                # ==========================================================================
                result = None

                if use_hybrid_extraction:
                    logger.info(f"[Phase2] Using WebAgent for: {candidate_url[:60]}")
                    ...
                    try:
                        ...
                        logger.info(
                            f"[Phase2] WebAgent result: determination={agent_result.determination}, "
                            f"products={len(agent_result.products)}"
                        )

                        # Build result based on determination
                        products = []
                        if agent_result.determination == "products_found":
                            for product in agent_result.products:
                                products.append({
                                    "name": product.name,
                                    "price": product.price if product.price else "N/A",
                                    "url": product.url,
                            ...
                            logger.info(f"[Phase2] ✓ WebAgent extracted {len(products)} products from {candidate_url[:60]}")
                            try:
                                from apps.services.tool_server.shared_state.site_health_tracker import get_health_tracker
                                ht = get_health_tracker()
                                ht.record_extraction(url=candidate_url, page_type="listing", success=True, method="phase2_web_agent")
                            except Exception as e:
                                logger.debug(f"[Phase2] Failed to record success: {e}")

                        elif agent_result.determination == "no_relevant_products":
                            # Valid result - page examined, nothing matches
                            logger.info(f"[Phase2] WebAgent found no relevant products: {agent_result.reason}")
                            try:
                                from apps.services.tool_server.shared_state.site_health_tracker import get_health_tracker
                                ht = get_health_tracker()
                                ht.record_extraction(url=candidate_url, page_type="listing", success=True, method="phase2_no_relevant")
                            except Exception as e:
                                logger.debug(f"[Phase2] Failed to record no_relevant: {e}")

                        else:
                            # Error/blocked
                            logger.warning(f"[Phase2] WebAgent failed: {agent_result.reason}")
                            try:
                                from apps.services.tool_server.shared_state.site_health_tracker import get_health_tracker
                                ht = get_health_tracker()
                                ht.record_extraction(url=candidate_url, page_type="listing", success=False, method="phase2_web_agent_error")
                            except Exception as e:
                                logger.debug(f"[Phase2] Failed to record error: {e}")
                        ...
                        logger.warning(f"[Phase2] WebAgent timed out for {candidate_url[:60]}")
                        continue
                    except Exception as web_agent_err:
                        logger.warning(f"[Phase2] WebAgent failed for {candidate_url[:60]}: {web_agent_err}")
                        continue
                else:
                    # LEGACY: Only used when PERCEPTION_ENABLE_HYBRID=false
                    # This should rarely happen - WebAgent is the default for Phase 2
                    logger.warning(f"[Phase2] WebAgent disabled, skipping {candidate_url[:60]}")
                    continue
...
async def _synthesize_findings(
    ...
    # Load prompt from recipe system
    try:
        recipe = load_recipe("research/synthesizer")
        base_prompt = recipe.get_prompt()
    except RecipeNotFoundError:
        logger.warning("[Synthesizer] Recipe not found, using fallback prompt")
...
async def _extract_intelligence_from_findings(
    ...
    # Load prompt from recipe system
    try:
        recipe = load_recipe("research/intelligence_synthesizer")
        base_prompt = recipe.get_prompt()
    except RecipeNotFoundError:
        logger.warning("[Intelligence] Recipe not found, using fallback prompt")
...
Return ONLY the JSON block from the "Structured Data" section. Do not include markdown formatting or explanation text."""
    ...
    if result["retailers"] and not result["retailers_mentioned"]:
        if isinstance(result["retailers"], dict):
            result["retailers_mentioned"] = list(result["retailers"].keys())
        elif isinstance(result["retailers"], list):
            result["retailers_mentioned"] = result["retailers"]
...
async def _generate_targeted_queries(
    ...
    """
    from apps.services.tool_server.query_planner import plan_phase2_queries
...
async def _web_vision_search_duckduckgo(
    ...
    try:
        ...
        # 0. Apply rate limiting - wait if needed to avoid triggering bot detection
        rate_limiter = get_search_rate_limiter()
        await rate_limiter.acquire(query, "DuckDuckGo")
        logger.info(f"[WebVisionSearch] Rate limiter cleared, proceeding with search")
        ...
        # 5b. Check for CAPTCHA/blocker page
        if page:
            ...
            page_data = {
                ...
                "url": current_url,
                "status": 200  # Playwright doesn't give us status easily, assume 200
            }
            blocker_info = detect_blocker(page_data)
            if blocker_info:
                blocker_type = blocker_info.get("type")
                indicators = blocker_info.get("indicators", [])
                logger.warning(f"[WebVisionSearch] Blocker detected: type={blocker_type}, indicators={indicators}")

                # Report to rate limiter so it backs off
                rate_limiter.report_rate_limit("DuckDuckGo")

                # Take screenshot for intervention UI
                screenshot_path = None
                try:
                    import os
                    screenshot_dir = "panda_system_docs/research_screenshots"
                    os.makedirs(screenshot_dir, exist_ok=True)
                    screenshot_path = f"{screenshot_dir}/ddg_blocker_{session_id}_{int(asyncio.get_event_loop().time())}.png"
                    await page.screenshot(path=screenshot_path)
                    logger.info(f"[WebVisionSearch] Saved blocker screenshot: {screenshot_path}")
                except Exception as e:
                    logger.warning(f"[WebVisionSearch] Failed to take screenshot: {e}")

                # Request human intervention with noVNC URL for live browser access
                blocker_type_str = blocker_type.value if hasattr(blocker_type, 'value') else str(blocker_type)
                novnc_url = "http://localhost:6080/vnc.html"
                logger.info(f"[WebVisionSearch] noVNC URL available: {novnc_url}")
                intervention = await request_intervention(
                    blocker_type=blocker_type_str,
                    url=current_url,
                    screenshot_path=screenshot_path,
                    session_id=session_id,
                    blocker_details={"indicators": indicators, "search_query": query},
                    cdp_url=novnc_url  # noVNC URL for live browser access
                )

                # Wait for user to resolve the CAPTCHA
                resolved = await intervention.wait_for_resolution(timeout=90)

                if resolved:
                    logger.info("[WebVisionSearch] Blocker resolved by user, continuing search")
                    # The user solved the CAPTCHA in the browser, results should now be visible
                    # Wait a moment for the page to update after CAPTCHA resolution
                    await asyncio.sleep(2)
                else:
                    logger.error("[WebVisionSearch] Blocker not resolved (timeout or skipped), aborting search")
                    return []
        ...
        from apps.services.tool_server.page_intelligence.service import PageIntelligenceService
            ...
            if anchor.startswith('.') or anchor.startswith('#') or anchor.startswith('['):
                ...
                if elems:
                    for elem in elems[:20]:  # Limit to 20 results
                        # Extract text AND URLs - get_text() strips URLs which we need!
                        # Find all links in this result element
                        ...
                        if text or urls_text:
                            organic_text += text + urls_text + '\n\n---\n\n'
                ...
                if elem:
                    # Same fix for non-CSS selectors
                    links = elem.find_all('a', href=True)
                    urls_text = ""
                    for link in links:
                        href = link.get('href', '')
...
async def _web_vision_search_google(
    ...
    try:
        ...
        while len(results) < max_results and pages_visited < max_pages:
            ...
            from apps.services.tool_server.web_vision_mcp import _get_or_create_page
...
async def _extract_with_recalibration_retry(
    ...
    **extract_kwargs
    ...
    """
    from apps.services.tool_server.shared_state.site_schema_registry import get_schema_registry
    from apps.services.tool_server.site_calibrator import get_calibrator
    from apps.services.tool_server.web_vision_mcp import _get_or_create_page
...
async def _extract_google_results_from_dom(
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════
        # JS-based extraction (inside-out approach)
        # Finds h3 elements and walks up to their anchor tags
        # ═══════════════════════════════════════════════════════════════
...
async def _extract_google_results_from_screenshot(
    ...
    try:
        ...
        # Load prompt from recipe system
        try:
            recipe = load_recipe("research/ocr_serp_analyzer")
            base_prompt = recipe.get_prompt()
        except RecipeNotFoundError:
            logger.warning("[ExtractGoogleOCR] Recipe not found, using fallback prompt")
...
async def _extract_google_results_from_content(
    ...
    try:
        # Load prompt from recipe system
        try:
            recipe = load_recipe("research/serp_analyzer")
            base_prompt = recipe.get_prompt()
        except RecipeNotFoundError:
            logger.warning("[ExtractGoogleResults] Recipe not found, using fallback prompt")
...
async def _summarize_page_content(
    ...
    if is_list_goal:
        ...
        # Load prompt from recipe system
        try:
            recipe = load_recipe("research/item_lister")
            base_prompt = recipe.get_prompt()
        except RecipeNotFoundError:
            logger.warning("[ContentSummarizer] item_lister recipe not found, using fallback prompt")
...
EXTRACTED ITEMS:"""
        ...
        # Load prompt from recipe system
        try:
            recipe = load_recipe("research/page_summarizer")
            base_prompt = recipe.get_prompt()
        except RecipeNotFoundError:
            logger.warning("[ContentSummarizer] page_summarizer recipe not found, using fallback prompt")
...
async def _extract_with_page_intelligence(
    ...
    try:
        ...
        from apps.services.tool_server.page_intelligence.models import ZoneType, PageType
        ...
        page_metadata = None  # Page metadata (pagination, reply counts, etc.)
        ...
        if target_zone and not use_prose_extraction:
            ...
            from apps.services.tool_server.page_intelligence.llm_client import get_llm_client
            ...
            # Preserve page metadata (pagination info, reply counts, etc.)
            page_metadata = extraction_result.get("page_metadata")
        ...
        # Include page metadata in summary if available (pagination, reply counts)
        if page_metadata:
            meta_parts = []
            if page_metadata.get("total_pages"):
                meta_parts.append(f"Total pages: {page_metadata['total_pages']}")
            if page_metadata.get("current_page"):
                meta_parts.append(f"Current page: {page_metadata['current_page']}")
            if page_metadata.get("total_replies"):
                meta_parts.append(f"Total replies: {page_metadata['total_replies']}")
            if meta_parts:
                summary_text = f"**Page Info:** {', '.join(meta_parts)}\n\n{summary_text}"
        ...
        # Include page metadata if extracted (pagination, reply counts, etc.)
        if page_metadata:
...
async def _web_vision_visit_and_read(
    ...
    try:
        ...
        from apps.services.tool_server.product_extractor import _extract_product_links
        ...
        if page_intel_result and page_intel_result.get("success"):
            ...
            # Include page metadata if available (pagination, reply counts)
            pi_page_metadata = page_intel_result.get("page_metadata")
            ...
            result = {
                ...
                "extracted_info": {
                    ...
                    "page_metadata": pi_page_metadata
                },
                "summary": pi_summary,
                "page_type": page_intel_result.get("page_type", "unknown"),
                ...
                "page_metadata": pi_page_metadata,  # Top-level for easy access
                "metadata": {
                    "extraction_method": "page_intelligence",
                    "extraction_goal": extraction_goal,
                    "zones_identified": page_intel_result.get("zones_identified", []),
...
async def _extract_search_results_from_content(
    ...
    # Load prompt from recipe system
    try:
        recipe = load_recipe("research/organic_results_extractor")
        base_prompt = recipe.get_prompt()
    except RecipeNotFoundError:
        logger.warning("[ExtractSearchResults] Recipe not found, using fallback prompt")
...
JSON OUTPUT:"""
    ...
    try:
        ...
        # LLM may return "results" or "organic_results" depending on prompt/context
        if isinstance(result, list):
            ...
            if "results" in result:
                ...
                candidates = result["organic_results"][:max_results]
            else:
                logger.warning(f"[ExtractSearchResults] Unexpected dict keys: {list(result.keys())}")
                candidates = []
        else:
...
async def _extract_information_llm(
    ...
    # Load prompt from recipe system
    try:
        recipe = load_recipe("research/webpage_reader")
        base_prompt = recipe.get_prompt()
    except RecipeNotFoundError:
        logger.warning("[ExtractInfo] Recipe not found, using fallback prompt")
