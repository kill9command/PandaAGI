...
_recipe_spec_cache: Dict[str, Dict[str, Any]] = {}


def load_prompt_via_recipe(recipe_name: str, category: str = "tools") -> str:
    """
...
# Backward-compatible alias (private name)
_load_prompt_via_recipe = load_prompt_via_recipe


# Recipe spec loader (for llm_params, token budgets, etc.)
def load_recipe_spec(recipe_name: str, category: str = "tools") -> Optional[Dict[str, Any]]:
    """
    Load raw recipe spec for parameter alignment.
    ...
    Returns:
        Raw recipe spec dict, or None if not found
    """
    cache_key = f"{category}/{recipe_name}"
    if cache_key in _recipe_spec_cache:
        return _recipe_spec_cache[cache_key]

    try:
        from libs.gateway.llm.recipe_loader import load_recipe
        recipe = load_recipe(f"{category}/{recipe_name}")
        spec = recipe._raw_spec or {}
        _recipe_spec_cache[cache_key] = spec
        return spec
    except Exception as e:
        logger.warning(f"[LLMUtils] Recipe spec {cache_key} not found: {e}")
        return None


# Backward-compatible alias (private name)
_load_recipe_spec = load_recipe_spec


# Legacy alias for backward compatibility
def _load_prompt(prompt_name: str) -> str:
    """Load prompt - maps legacy names to recipe names."""
    return load_prompt_via_recipe(prompt_name, "tools")
...
BEGIN SUMMARY:"""
    ...
    recipe_spec = _load_recipe_spec("content_summarizer")
    llm_params = recipe_spec.get("llm_params", {}) if recipe_spec else {}
    max_tokens_limit = llm_params.get("max_tokens")
    max_tokens_default = target_content_tokens + 50  # Small buffer
    max_tokens = (
        min(int(max_tokens_limit), int(max_tokens_default))
        if max_tokens_limit
        else int(max_tokens_default)
    )
