...
class TurnStore:
    ...
    Example usage:
        ...
        # Load a turn
        loaded = await store.load_turn("default", 1)

        # List turns
        summaries = await store.list_turns("default", limit=10)
    """
        ...
        return Turn(
            ...
            metadata={k: v for k, v in metadata.items()
                     if k not in ("turn_number", "session_id", "turn_dir",
                                  "created_at", "topic", "action_needed", "intent", "quality")},
    ...
    async def update_turn_metadata(
        ...
        Args:
            ...
            action_needed: New action_needed value (optional)
        ...
        if action_needed is not None:
            metadata["action_needed"] = action_needed
