...
from apps.services.tool_server.shared_state.topic_index import TopicIndex, Topic, get_topic_index
...
JSON response:"""
    ...
    async def tag_claims_with_topic(
        ...
        # Fetch all active claims once (avoid O(n*m) repeated listing)
        all_claims_map = {}
        if auto_categorize:
            all_claims_map = {c.claim_id: c for c in self.claim_registry.list_active_claims()}

        for claim_id in claim_ids:
            update = {"claim_id": claim_id, "topic_id": topic_id}

            if auto_categorize:
                claim = all_claims_map.get(claim_id)
                if claim:
                    claim_type = ClaimType.from_statement(claim.statement)
