"""Tool Server configuration module.

Architecture Reference:
    architecture/main-system-patterns/services/tool-execution-service.md

Tool Server Configuration:
    - TOOL_SERVER_HOST: Host to bind Tool Server (default: 0.0.0.0)
    - TOOL_SERVER_PORT: Port to bind Tool Server (default: 8090)
...
class ToolServerConfig(BaseSettings):
    """Tool Server service configuration.

    The Tool Server executes MCP tools on behalf of the Gateway.
    The Gateway's Coordinator (Phase 5) calls the Tool Server for tool execution.
    """
...
@lru_cache()
def get_config() -> ToolServerConfig:
    """Get cached Tool Server configuration."""
