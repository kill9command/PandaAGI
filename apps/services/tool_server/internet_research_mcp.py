"""
apps/services/tool_server/internet_research_mcp.py

Internet Research MCP Tool - Entry point for the internet.research tool.

This module provides the /internet.research endpoint handler that uses the
LLM-driven research API for multi-phase web research:

Flow:
    /internet.research endpoint (apps/tool_server/app.py)
        → adaptive_research() (this file)
        → execute_full_research() (apps/tools/internet_research/)
            → Phase 1: LLM-driven search and visit loop
            → Phase 2: Product extraction from vendors

See: architecture/mcp-tool-patterns/internet-research-mcp/INTERNET_RESEARCH_ARCHITECTURE.md

Created: 2025-11-21
Updated: 2026-01-27 - Migrated to LLM-driven apps/tools/internet_research API
"""
...
async def adaptive_research(
    ...
    turn_dir_path: Optional[str] = None  # Turn directory for Document IO compliance
) -> Dict[str, Any]:
    """
    ...
    Uses the LLM-driven apps/tools/internet_research API for autonomous
    Google search, CAPTCHA handling, and LLM-based content extraction.
    ...
    if research_context:
        # Log new user_purpose fields
        action_needed = research_context.get('action_needed', 'unknown')
        data_reqs = research_context.get('data_requirements', {})
        user_purpose = research_context.get('user_purpose', '')[:80]
        ...
        logger.info(
            ...
            f"needs_prices={data_reqs.get('needs_current_prices', False)}, "
            f"user_purpose={user_purpose}..., "
            f"legacy_intent={intent}, "
    ...
    if event_emitter:
        await event_emitter.emit("research_started", {
            ...
            "strategy": "llm_driven_research"
        })

    try:
        # Use new LLM-driven research API directly
        from apps.tools.internet_research import execute_full_research

        # Extract intent from research_context
        # NEW: Prefer data_requirements.needs_current_prices over legacy intent
        intent = "commerce"  # Default to commerce for backward compatibility
        if research_context:
            # Check new fields first (from Phase 0 user_purpose system)
            data_requirements = research_context.get("data_requirements", {})
            action_needed = research_context.get("action_needed", "")

            if data_requirements.get("needs_current_prices"):
                intent = "commerce"
            elif action_needed == "navigate_to_site":
                intent = "informational"  # Navigation uses Phase 1 only
            elif action_needed == "live_search":
                # Check if it's truly informational (no price needs)
                if not data_requirements.get("needs_live_data"):
                    intent = "informational"
                else:
                    # Fall back to legacy intent
                    intent_from_ctx = research_context.get("intent", "commerce")
                    if intent_from_ctx in ("navigation", "site_search"):
                        intent = "informational"
                    elif intent_from_ctx in ("informational", "commerce"):
                        intent = intent_from_ctx
            else:
                # Fall back to legacy intent for backward compatibility
                intent_from_ctx = research_context.get("intent", "commerce")
                if intent_from_ctx in ("navigation", "site_search"):
                    intent = "informational"
                elif intent_from_ctx in ("informational", "commerce"):
        ...
        # Extract context for the research
        # NEW: Prefer user_purpose over prior_turn_context
        prior_context = ""
        if research_context:
            user_purpose = research_context.get("user_purpose", "")
            prior_turn_ctx = research_context.get("prior_turn_context", "")
            prior_context_info = research_context.get("prior_context", {})

            # Combine user_purpose with prior context for richer understanding
            context_parts = []
            if user_purpose:
                context_parts.append(f"User Purpose: {user_purpose}")
            if prior_context_info.get("prior_turn_purpose"):
                context_parts.append(f"Prior Turn: {prior_context_info['prior_turn_purpose']}")
            if prior_turn_ctx:
                context_parts.append(prior_turn_ctx)
            prior_context = "\n".join(context_parts)

        raw_result = await execute_full_research(
            goal=query,
            intent=intent,
        ...
        # Transform new API result to Gateway-compatible format
        phase1_data = raw_result.get("phase1", {}) or {}
        phase2_data = raw_result.get("phase2", {}) or {}

        # Build research_result in the format downstream code expects
        research_result = {
            "query": query,
            "mode": mode,
            "strategy_used": "llm_driven_research",
            "strategy_reason": f"LLM-driven research ({intent} intent)",
            "passes": 1,
            "results": {
                "findings": raw_result.get("products", []),
                "phase1_only": intent == "informational" or not raw_result.get("products"),
            },
            "findings": raw_result.get("products", []),  # Top-level for navigation strategy
            "intelligence": phase1_data.get("intelligence", {}),
            "phase1_sources": [{"url": u} for u in phase1_data.get("sources", [])],
            "intelligence_cached": False,
            "stats": {
                "sources_visited": phase1_data.get("pages_visited", 0) + len(phase2_data.get("vendors_visited", [])),
                "total_sources": phase1_data.get("pages_visited", 0) + len(phase2_data.get("vendors_visited", [])),
                "sources_extracted": len(raw_result.get("products", [])),
            },
        }

        # For informational queries, convert findings to statements
        if intent == "informational" and phase1_data.get("findings"):
            research_result["findings"] = [
                {"statement": f, "type": "fact", "confidence": 0.8, "source": "phase1_intelligence"}
                for f in phase1_data.get("findings", [])
            ]
            research_result["results"]["findings"] = research_result["findings"]
...
# ============================================================================
#
# 2026-01-27: Migrated to LLM-driven apps/tools/internet_research API
#   - adaptive_research() now calls execute_full_research() directly
#   - Bypasses research_role.py and research_orchestrator.py (now archived)
#   - New API: ~2,000 lines vs old ~9,500 lines
#
# The new API provides:
#   - LLM decides all strategy (search/visit/done loop)
...
async def trigger_captcha_intervention(
    ...
    """
    try:
        from apps.services.tool_server.captcha_intervention import queue_intervention
