...
def _load_prompt_via_recipe(recipe_name: str, category: str = "tools") -> str:
    ...
    Args:
        recipe_name: Name of the recipe (e.g., "pdp_selector")
    ...
    try:
        ...
        logger.warning(f"[PDPExtractor] Recipe {cache_key} not found: {e}")
        return ""
...
class PDPExtractor:
    ...
    async def _extract_with_html(self, page: 'Page', url: str) -> Optional[PDPData]:
        ...
        try:
            ...
            from apps.services.orchestrator.page_intelligence.legacy_adapter import get_smart_calibrator
    ...
    async def _calibrate_pdp(self, page: 'Page', url: str, calibrator) -> Optional['ExtractionSchema']:
        ...
        page_info = await page.evaluate("""() => {
            ...
            // Check for hidden a11y price (Amazon pattern)
            const hiddenPrice = document.querySelector('.a-offscreen');
            if (hiddenPrice && priceRegex.test(hiddenPrice.textContent?.trim() || '')) {
                result.priceElements.unshift({
                    selector: '.a-offscreen',
                    text: hiddenPrice.textContent.trim(),
                    tag: 'span',
                    classes: 'a-offscreen',
                    y: 0,
                    note: 'Amazon-style hidden accessible price'
                });
            }
...
4. If multiple prices, choose the one closest to the cart button
5. Return empty string "" if you can't determine a selector
6. NEVER use CSS-in-JS hashed class names like "-sc-abc123" or "css-xyz789"
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if start >= 0 and end > start:
                    ...
                    # Using PageIntelligence adapter for backwards compatibility
                    from apps.services.orchestrator.page_intelligence.legacy_adapter import ExtractionSchema
