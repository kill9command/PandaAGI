...
from apps.services.orchestrator.source_quality_scorer import get_source_quality_scorer
...
def _load_vendor_selector_prompt() -> str:
    """Load vendor_selector prompt via recipe system."""
    global _vendor_selector_prompt
    if _vendor_selector_prompt is None:
        try:
            from libs.gateway.recipe_loader import load_recipe
            recipe = load_recipe("tools/vendor_selector")
            _vendor_selector_prompt = recipe.get_prompt()
            logger.info("[VendorSelect] Loaded vendor_selector prompt from recipe")
        except Exception as e:
            logger.warning(f"[VendorSelect] Recipe load failed: {e}, using fallback")
...
async def select_vendor_candidates(
    ...
    try:
        from apps.services.orchestrator.shared_state.vendor_registry import VendorRegistry
