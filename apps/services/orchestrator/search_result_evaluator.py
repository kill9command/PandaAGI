...
def _load_search_result_evaluator_prompt() -> str:
    """Load the search result evaluator prompt from recipe system."""
    ...
    if _search_result_evaluator_prompt is None:
        try:
            ...
            recipe = load_recipe("research/search_result_evaluator")
            _search_result_evaluator_prompt = recipe.get_prompt()
            logger.info("[SearchResultEvaluator] Loaded prompt from recipe system")
        except Exception as e:
            logger.warning(f"[SearchResultEvaluator] Recipe load failed: {e}, using fallback")
