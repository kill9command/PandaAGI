...
Executes the 8-phase pipeline with proper loop control for
...
8-Phase Pipeline:
    ...
    Phase 1: Reflection (REFLEX role, temp=0.3) - PROCEED/CLARIFY gate
    Phase 2: Context Gatherer (MIND role, temp=0.5)
    Phase 3: Planner (MIND role, temp=0.5)
    Phase 4: Coordinator (MIND role, temp=0.5) - tool execution
    Phase 5: Synthesis (VOICE role, temp=0.7)
    Phase 6: Validation (MIND role, temp=0.5) - quality gate
    Phase 7: Save (procedural, no LLM)
...
from apps.services.orchestrator.services.session_manager import SessionManager, get_session_manager
...
class PipelineService:
    """
    Orchestrates the 8-phase pipeline execution.
