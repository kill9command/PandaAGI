...
def _load_prompt_via_recipe(recipe_name: str, category: str = "tools") -> str:
    ...
    Args:
        recipe_name: Name of the recipe (e.g., "viability_evaluator_v2")
    ...
    try:
        ...
        logger.warning(f"[ProductViability] Recipe {cache_key} not found: {e}")
        return ""
...
Evaluate each product against the requirements and determine viability."""
    ...
    try:
        ...
        if rejected:
            try:
                from apps.services.orchestrator.rejection_tracker import get_rejection_tracker
