...
from apps.services.orchestrator.shared import call_llm_text
...
Now evaluate and respond:"""
    ...
    def _parse_meta_response(self, response: str, role: str) -> MetaReflectionResult:
        ...
        query_type = None
        query_type_match = re.search(r'QUERY_TYPE:\s*(ACTION|RECALL|INFORMATIONAL|CLARIFICATION)', response, re.IGNORECASE)
        if query_type_match:
    ...
    def write_document(self, result: MetaReflectionResult, turn_dir: 'TurnDirectory') -> 'pathlib.Path':
        ...
        """
        import pathlib
        from libs.gateway.doc_writers import write_meta_reflection_md
