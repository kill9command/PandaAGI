...
- `ok`: New actionable claims present (including diagnostic claims about failures).
- `empty`: Nothing useful AND no diagnostic information; explain why in caveats.
- `conflict`: Conflicting evidence; emit all sides with lowered confidence.
- `error`: Bundle malformed; brief explanation.

### Zero-Result Handling (NEW)

When all tools return zero results:
1. Set status to **`ok`** (NOT `empty`) if you can create diagnostic claims
2. Create claims like:
   - "No offers found for '[query]' with [details]"
   - "Research search returned 0 results. Possible issue: [issue]"
   - "Suggested refinement: [refinement suggestion]"
3. Include suggested next actions in `open_questions`
4. Set `quality_score` to 0.0-0.3 (low quality due to no results)
5. Extract failure reasons from tool metadata (`issues`, `suggested_refinement`)

**Example for zero-result case:**
```json
{
  "_type": "CAPSULE",
  "status": "ok",
  "claims": [
    {
      "claim": "No offers found for 'Syrian hamster'. Suggestion: broaden search or try different query",
      "evidence": ["tk_123:commerce.search_offers:0"],
      "confidence": "high",
      "last_verified": "2025-11-11"
    }
  ],
  "caveats": ["All search strategies returned zero results"],
  "open_questions": ["Should user try broader search terms?", "Are there alternative product names?"],
  "quality_report": {
    "quality_score": 0.2,
    "meets_threshold": false,
    "rejection_breakdown": {"zero_results_all_strategies": 1},
    "suggested_refinement": "Broaden query or remove negative filters"
  }
}
```

## Product Research Findings (internet.research)

When processing `internet.research` results with product findings:

**CRITICAL:** Extract each finding as a separate claim with structured metadata.
...
**Extraction Rules:**
1. **One claim per product** - Don't merge multiple products into one claim
2. **Include URL in statement** - URLs are CRITICAL for user action
3. **Preserve price** - Include exact price when available
4. **Keep vendor** - Include vendor/site name
5. **Confidence mapping**: finding.confidence → claim.confidence

**Claim Format for Products:**
```json
{
  "claim": "{ProductName} at {Vendor} for {Price} - {URL}",
  "evidence": ["tk_123:internet.research:0"],
  "confidence": "high",
  ...
  "findings": [
    {
      ...
      "price": "$2,799.99",
      "vendor": "rog.asus.com",
      "url": "https://rog.asus.com/us/laptops/rog-zephyrus/...",
      "description": "RTX 5090 GPU, Intel Core Ultra 9, 2.5K OLED 240Hz",
      "confidence": 0.8
    }
  ]
}
...
```json
{
  "claim": "ASUS ROG Zephyrus G16 (2025) at rog.asus.com for $2,799.99 - https://rog.asus.com/us/laptops/rog-zephyrus/...",
  "evidence": ["tk_123:internet.research:0"],
  "confidence": "high",
  "last_verified": "2025-11-24",
  "metadata": {
    "product_name": "ASUS ROG Zephyrus G16 (2025)",
    "price": "$2,799.99",
    "vendor": "rog.asus.com",
    "url": "https://rog.asus.com/us/laptops/rog-zephyrus/...",
    "description": "RTX 5090 GPU, Intel Core Ultra 9, 2.5K OLED 240Hz"
...
- Has price + URL + vendor: quality_score ≥ 0.8
- Has URL + description: quality_score ≥ 0.7
- Missing URL: quality_score ≤ 0.5 (not actionable)

**Token Budget Management:**
- Max 15 claims total (per memory caps rule)
- Prioritize products with:
  1. Complete information (price + URL + description)
  2. Higher confidence scores
  3. Better vendor reputation
