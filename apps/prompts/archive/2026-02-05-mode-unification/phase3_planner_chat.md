...
**IMPORTANT - Follow-up Queries with URLs:**
If §0 shows a `content_reference.source_url` for a resolved reference, route to `executor` to visit that URL
and get detailed information. Do NOT route to `synthesis` with generic information when a specific URL is available.
