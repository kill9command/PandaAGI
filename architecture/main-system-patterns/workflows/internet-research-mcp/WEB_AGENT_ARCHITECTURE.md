...
WebAgent is called from Phase 2 product finding (`apps/tools/internet_research/phase2_products.py`):

```python
async def _extract_from_vendor(self, vendor_url: str, goal: str, search_terms: list[str]) -> list[Product]:
    ...
    result: WebAgentResult = await agent.navigate(
        url=search_url,
    ...
    # Handle determination signals
    if result.determination == "products_found":
        return self._convert_webagent_products(result.products, vendor_domain)
    elif result.determination == "no_relevant_products":
        return []  # Valid result - no matching products
    elif result.determination == "blocked":
        return []  # Intervention was attempted
    # ... other determinations
```

**No fallbacks.** If WebAgent fails to extract products, it creates an intervention. Phase 2 does not try alternative extraction methods - it respects WebAgent's determination signals.
...
| File | Change |
|------|--------|
| `apps/tools/internet_research/phase2_products.py` | **DONE** - Uses WebAgent for vendor extraction |
| `apps/services/orchestrator/archive/research_orchestrator.py` | Moved to archive, marked deprecated |

### Deprecated Files

| File | Status |
|------|--------|
| `apps/services/orchestrator/archive/research_orchestrator.py` | Archived, marked deprecated |
...
### Phase 1: Create WebAgent ✅ COMPLETE
...
### Phase 2: Update Phase 2 Product Finding ✅ COMPLETE (2026-02-02)
1. ✅ Replace vendor visit logic with WebAgent calls in `phase2_products.py`
2. ✅ Handle WebAgent determination signals properly
3. ✅ Intervention creation via WebAgent's built-in system

### Phase 3: Verify and Cleanup
1. [ ] Run hamster and laptop golden queries
2. [ ] Verify no fallback code paths remain
3. [x] Old research_orchestrator moved to archive/
...
- [x] ONE code path for all vendor extraction (WebAgent in phase2_products.py)
- [x] Stuck detection prevents infinite loops
- [x] Site knowledge improves over time (18 domain files in panda_system_docs/site_knowledge/)
...
- [x] Phase 2 uses WebAgent (no hardcoded LLM text extraction)
...
- [phase4-executor.md](../../main-system-patterns/phase4-executor.md) - Tool execution phase
- [phase5-coordinator.md](../../main-system-patterns/phase5-coordinator.md) - Tool coordination phase
