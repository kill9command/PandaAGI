"""
orchestrator/llm_candidate_filter.py
...
from orchestrator.source_quality_scorer import get_source_quality_scorer
...
_CANDIDATE_FILTER_PROMPT_PATH = _PROMPTS_DIR / "candidate_filter.md"
_VENDOR_VALIDATOR_PROMPT_PATH = _PROMPTS_DIR / "vendor_validator.md"
...
def _load_candidate_filter_prompt() -> str:
    """Load the candidate filter prompt from file."""
    global _candidate_filter_prompt
    if _candidate_filter_prompt is None:
        if _CANDIDATE_FILTER_PROMPT_PATH.exists():
            _candidate_filter_prompt = _CANDIDATE_FILTER_PROMPT_PATH.read_text()
            logger.info(f"[CandidateFilter] Loaded prompt from {_CANDIDATE_FILTER_PROMPT_PATH}")
        else:
            logger.warning(f"[CandidateFilter] Prompt file not found: {_CANDIDATE_FILTER_PROMPT_PATH}")
...
def _load_vendor_validator_prompt() -> str:
    """Load the vendor validator prompt from file."""
    global _vendor_validator_prompt
    if _vendor_validator_prompt is None:
        if _VENDOR_VALIDATOR_PROMPT_PATH.exists():
            _vendor_validator_prompt = _VENDOR_VALIDATOR_PROMPT_PATH.read_text()
            logger.info(f"[VendorValidator] Loaded prompt from {_VENDOR_VALIDATOR_PROMPT_PATH}")
        else:
            logger.warning(f"[VendorValidator] Prompt file not found: {_VENDOR_VALIDATOR_PROMPT_PATH}")
...
def _load_vendor_selector_recipe():
    ...
    if _vendor_selector_recipe is None:
        try:
            from lib.gateway.recipe_loader import load_recipe
...
def _load_vendor_selector_prompt() -> str:
    """Load vendor_selector prompt template."""
    global _vendor_selector_prompt
    if _vendor_selector_prompt is None:
        prompt_path = Path("apps/prompts/vendor_selector/core.md")
        try:
            with open(prompt_path, 'r') as f:
                _vendor_selector_prompt = f.read()
            logger.info("[VendorSelect] Loaded vendor_selector prompt")
        except FileNotFoundError:
            logger.warning(f"[VendorSelect] Prompt not found: {prompt_path}")
            _vendor_selector_prompt = "Select vendors from search results. Return JSON."
    return _vendor_selector_prompt
...
async def select_vendor_candidates(
    ...
    try:
        from orchestrator.shared_state.vendor_registry import VendorRegistry
