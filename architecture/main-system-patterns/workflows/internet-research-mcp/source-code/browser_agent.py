...
UPDATED: 2025-11-20 - Now uses Web Vision MCP instead of human_web_navigator
...
from orchestrator.shared import call_llm_json, call_llm_text

logger = logging.getLogger(__name__)

# Prompt file paths
_PROMPTS_DIR = Path(__file__).parent.parent / "apps" / "prompts" / "browser"


def _load_prompt(name: str) -> str:
    """Load a prompt from file with graceful fallback."""
    prompt_path = _PROMPTS_DIR / f"{name}.md"
    if prompt_path.exists():
        return prompt_path.read_text()
    else:
        logger.warning(f"[BrowserAgent] Prompt file not found: {prompt_path}")
        return ""
...
async def detect_navigation_opportunities(
    ...
    base_prompt = _load_prompt("pagination")
    if not base_prompt:
...
async def aggregate_multi_page_results(
    ...
    # Load prompt and build context
    base_prompt = _load_prompt("multi_page_synth")
    if not base_prompt:
        # Fallback if prompt file not found
...
async def should_continue_browsing(
    ...
    # Load prompt and build context
    base_prompt = _load_prompt("continuation")
    if not base_prompt:
        # Fallback if prompt file not found
...
async def _check_relevance_llm(
    ...
    # Load prompt and build context
    base_prompt = _load_prompt("relevance")
    if not base_prompt:
        # Fallback if prompt file not found
...
def _check_content_similarity(new_page: Dict, previous_pages: List[Dict]) -> float:
    ...
    """
    try:
        from orchestrator.shared_state.embedding_service import EMBEDDING_SERVICE
