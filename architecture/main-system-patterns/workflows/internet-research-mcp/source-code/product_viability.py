...
_PROMPT_DIR = Path(__file__).parent.parent / "apps" / "prompts" / "product"
_prompt_cache: Dict[str, str] = {}


def _load_prompt(prompt_name: str) -> str:
    """Load a prompt file from the product prompts directory."""
...
async def filter_viable_products(
    ...
    prompt_path = Path(__file__).parent.parent / "apps" / "prompts" / "product" / "viability_evaluator.md"
    if prompt_path.exists():
...
async def filter_viable_products_with_reasoning(
    ...
    # Load viability reasoning prompt template
    ...
    try:
        ...
        logger.error(f"[Viability:Reasoning] Prompt template not found: {prompt_template_path}")
            ...
            if 0 <= product_idx < len(products):
                ...
                if decision == "ACCEPT":
                    ...
                    logger.info(
                        ...
                        f"(score: {product['viability_score']:.2f})"
                    )
