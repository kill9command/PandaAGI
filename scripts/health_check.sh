...
# Pandora stack health check (vLLM, Gateway, Orchestrator)
...
check_orchestrator_docsearch() {
  local url="${ORCH_URL}/doc.search"
  local payload='{"query":"README","k":1}'
  local out; out=$(curl -sS -m 20 -H 'Content-Type: application/json' -d "$payload" "$url" || true)
  if echo "$out" | grep -q '"summary"'; then ok "Orchestrator /doc.search"; else bad "Orchestrator /doc.search ($out)"; fi
}
...
check_gateway_policy
check_gateway_chat
hr; check_orchestrator_docsearch
