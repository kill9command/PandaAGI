...
async def set_playwright_headless_mode(mode: str):
    ...
    env_path = Path("/path/to/pandaagi/.env")
...
async def restart_orchestrator():
    ...
    os.system("ps aux | grep uvicorn | grep tool_server | grep -v grep | awk '{print $2}' | xargs kill 2>/dev/null")
    await asyncio.sleep(2)

    # Start new orchestrator
    os.system("nohup uvicorn apps.tool_server.app:app --host 127.0.0.1 --port 8090 >tool_server.log 2>&1 &")
    await asyncio.sleep(5)

    print("[System] Tool Server restarted")
