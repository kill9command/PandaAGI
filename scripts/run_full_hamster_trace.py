...
Usage:
  set -a && source .env && PYTHONPATH=/path/to/pandaagi conda run -n panda python scripts/run_full_hamster_trace.py
...
def run_discovery_and_stage(queries: List[str]) -> None:
    ...
    try:
        from apps.services.tool_server import context_builder
    except Exception as e:
        print("Failed to import apps.services.tool_server.context_builder:", e)
        traceback.print_exc()
        raise
...
def call_solver_with_facts(facts: List[str]) -> None:
    ...
    model_id = os.environ.get("THINK_MODEL_ID") or os.environ.get("SOLVER_MODEL_ID") or "/path/to/pandaagi/models/solver"
