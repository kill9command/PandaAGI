...
class ContextDocument:
    ...
    def __init__(self, turn_number: int, session_id: str, query: str):
        ...
        self.sections: Dict[int, dict] = {}  # §1-5 appended by each phase

    def append_section(self, section_num: int, title: str, content: str):
        """Append a new section to the document (§1-5 only)."""
        if section_num < 1 or section_num > 5:
            raise ValueError("Sections must be 1-5 (§0 is query, set at init)")
...
def simulate_phase_1_context_gatherer(doc: ContextDocument):
    """Simulate Context Gatherer appending §1."""
...
| sessions/henry/preferences.md | 0.88 | User preferences including favorite_hamster |

**Source References:**
- [1] turns/turn_000620/context.md - "User's favorite hamster preference"
- [2] sessions/henry/preferences.md - "Stored user preferences\""""

    doc.append_section(1, "Gathered Context", content)


def simulate_phase_2_reflection(doc: ContextDocument):
    """Simulate Reflection appending §2."""
...
def simulate_phase_5_synthesis(doc: ContextDocument):
    """Simulate Synthesis appending §5."""
...
- [x] Appropriate format (simple acknowledgment)"""
    ...
    doc.append_section(5, "Synthesis", content)
...
def main():
    ...
    print("\n[Phase 1] Context Gatherer appends §1")
    simulate_phase_1_context_gatherer(doc)

    # Phase 2: Reflection
    print("[Phase 2] Reflection appends §2")
    simulate_phase_2_reflection(doc)
    ...
    # Phase 5: Synthesis
    print("[Phase 5] Synthesis appends §5")
    simulate_phase_5_synthesis(doc)
