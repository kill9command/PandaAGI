...
EVAL_TIME=$(tail -200 orchestrator.log | grep "\[Phase 3\] Evaluating" | tail -1 || true)
if [ -n "$EVAL_TIME" ]; then
...
echo "2. Check orchestrator.log for Phase 3 decision making"
echo "3. Monitor latency with: tail -f orchestrator.log | grep '\[Phase 3\]'"
