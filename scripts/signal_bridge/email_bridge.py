#!/usr/bin/env python3
"""
Email Bridge for Claude Code / Panda

Monitors an email inbox and forwards messages to Claude Code or Panda.
...
Usage:
    ...
    python email_bridge.py --mode panda   # Forward to Panda Gateway
"""
...
# Panda Gateway
PANDA_URL = "http://127.0.0.1:9000/chat"
...
def handle_panda(msg: EmailMessage) -> str:
    """Forward to Panda Gateway."""
    try:
        import httpx
        ...
        return result.get("response", "No response from Panda")

    except Exception as e:
        return f"Error calling Panda: {e}"


def main():
    parser = argparse.ArgumentParser(description="Email Bridge for Claude/Panda")
    parser.add_argument(
        "--mode",
        choices=["echo", "claude", "panda"],
        default="echo",
        help="Operation mode"
    )
    parser.add_argument(
