...
def handle_panda(sender: str, text: str):
    ...
    try:
        ...
        reply = result.get("response", "No response from Panda")
        send_signal(sender, reply)
        print(f"  Panda responded with {len(reply)} chars")

    except Exception as e:
        error_msg = f"Error calling Panda: {e}"
