...
def record_lesson(
    ...
    response = httpx.post(
        f"{ORCH_URL}/reflection.record_lesson",
        json={
            "role": role,
            "context": context,
            "outcome": outcome,
...
def query_lessons(
    ...
    response = httpx.post(
        f"{ORCH_URL}/reflection.query_lessons",
        json={
            "role": role,
            "tags": tags,
...
def get_stats() -> Dict[str, Any]:
    ...
    response = httpx.post(f"{ORCH_URL}/reflection.get_stats", timeout=30.0)
    response.raise_for_status()
