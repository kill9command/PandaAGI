...
from .context_document import ContextDocument, TurnMetadata, extract_keywords
from .turn_search_index import TurnSearchIndex

# Optional import for scope promotion (#71)
try:
    from .research_index_db import get_research_index_db
...
try:
    ...
    from .doc_pack_builder import DocPackBuilder
...
class TurnSaver:
    ...
    async def save_turn(
        ...
        metadata = self._generate_metadata(
            context_doc, response, response_quality, validation_result
        )
    ...
    def _generate_metadata(
        ...
        # Extract intent from task plan section
        intent = self._extract_intent(context_doc)

        # Extract tools used from tool execution section
        tools_used = self._extract_tools_used(context_doc)
    ...
    def _extract_intent(self, context_doc: ContextDocument) -> str:
        """
        Extract intent from the context document.
    ...
    def _extract_tools_used(self, context_doc: ContextDocument) -> List[str]:
        """Extract tools used from the tool execution section."""
        tool_execution = context_doc.get_section(4)
        if not tool_execution:
            return []

        tools = []
        for line in tool_execution.split("\n"):
            # Look for "- `tool_name`:" format
    ...
    async def _update_persistent_memory(
        ...
        # Check for intent = preference
        if intent == "preference" and not preferences_to_save:
    ...
    def _is_memorable_turn(
        ...
        # Check intent
        intent = metadata.intent if metadata else self._extract_intent(context_doc)
        memorable_intents = {"transactional", "informational", "research", "commerce"}

        if intent in memorable_intents:
            return True
    ...
    async def _process_learning(
        ...
        from .performance_tracker import TurnOutcome
        import time
