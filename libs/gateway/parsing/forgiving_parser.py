...
class ForgivingParser:
    ...
    def parse(
        ...
        logger.error("ForgivingParser: all parse strategies failed, defaulting to schema defaults", extra={"raw": raw_output[:500] if raw_output else ""})
        return ParseResult(
            data=defaults,
            success=False,
...
def parse_phase1_output(raw: str) -> ParseResult:
    ...
    if not result.data.get("decision"):
        ...
        if "CLARIFY" in raw_upper:
            ...
            logger.error("ForgivingParser: phase1 reflection decision unparseable, defaulting to PROCEED", extra={"phase": "phase1", "raw": raw[:500] if raw else ""})
...
def parse_phase3_output(raw: str) -> ParseResult:
    ...
    if not result.data.get("route_to"):
        ...
        if "synthesis" in raw_lower or "direct" in raw_lower:
            ...
            logger.error("ForgivingParser: phase3 planner route unparseable, defaulting to executor", extra={"phase": "phase3", "raw": raw[:500] if raw else ""})
...
def parse_phase4_output(raw: str) -> ParseResult:
    ...
    if not result.data.get("action"):
        ...
        if "EXECUTE" in raw_upper or "COMMAND" in raw_upper:
            ...
            logger.error("ForgivingParser: phase4 executor action unparseable, defaulting to COMPLETE", extra={"phase": "phase4", "raw": raw[:500] if raw else ""})
...
def parse_phase6_output(raw: str) -> ParseResult:
    ...
    if not result.data.get("decision"):
        ...
        if "RETRY" in raw_upper:
            ...
            logger.error("ForgivingParser: phase6 validation decision unparseable, defaulting to APPROVE", extra={"phase": "phase6", "raw": raw[:500] if raw else ""})
