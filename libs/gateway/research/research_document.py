...
Design Notes:
- DECAY_RATES and CONFIDENCE_FLOOR below are module-level defaults. For
  consistent confidence handling across the system, prefer using
  libs.gateway.validation.confidence_calibration which provides the
  canonical source of truth (used by research_index_db.py).
- CONTENT_TYPES keywords are used for document indexing/search optimization,
  not for subjective relevance decisions. LLM-driven classification should
  be used for routing and intent decisions (see Phase 3 Planner).
"""
...
# =============================================================================
#
# NOTE: These are legacy module-level defaults. For consistency, prefer using
# libs.gateway.validation.confidence_calibration which is the canonical source
# of truth for confidence decay (used by research_index_db.py).
...
# =============================================================================
#
# NOTE: These keyword lists are used for document indexing and search
# optimization (objective classification of what information is present).
# They are NOT used for subjective relevance decisions. Per architecture
# guidelines, LLM-driven evaluation should be used for relevance scoring
# and intent-level routing decisions (see Phase 3 Planner).
        ...
        for finding in findings:
            ...
            if vendor_name not in vendors_seen:
                ...
                vendors_seen[vendor_name] = VendorInfo(
                    ...
                    notes=(finding.get("description") or "")[:100]
                )
