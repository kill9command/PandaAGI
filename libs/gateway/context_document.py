...
Sections:
- §0: User Query (immutable, set at init)
- §1: Gathered Context (Context Gatherer)
- §2: Reflection Decision (Reflection)
- §3: Task Plan (Planner)
- §4: Tool Execution (Coordinator) - optional
- §5: Synthesis (Synthesis)
- §6: Validation (Validator) - verifies response before sending
"""
...
class ContextDocument:
    ...
    - Header: Turn number, session ID
    - §0: User Query (immutable)
    ...
    Usage:
        ...
        ...
        doc.append_section(6, "Validation", content)
        doc.save(turn_dir)
    """
    ...
    def __init__(self, turn_number: int, session_id: str, query: str):
        ...
        # Full query analysis from Phase 0 (THE SOURCE OF TRUTH)
        # Contains: user_purpose, action_needed, data_requirements, prior_context, mode
        # See: architecture/plans/INTENT_SYSTEM_MIGRATION.md
    ...
    def set_section_0(self, query_analysis: Dict[str, Any]):
        ...
        This is the canonical way to store Phase 0 output. All downstream phases
        should read user_purpose from here using get_user_purpose(), get_action_needed(), etc.
        ...
        Args:
            query_analysis: Dict containing:
                ...
                - action_needed: str (live_search, recall_memory, answer_from_context, etc.)
                - data_requirements: dict (needs_current_prices, needs_product_urls, etc.)
                - prior_context: dict (continues_topic, relationship, etc.)
                - mode: str (chat, code)
    ...
    def _format_section_0(self) -> str:
        ...
        # Action needed and data requirements
        lines.append(f"**Action Needed:** {qa.get('action_needed', 'unclear')}")
        ...
        # Prior context (how this relates to previous turns)
        prior_ctx = qa.get('prior_context', {})
        if prior_ctx and prior_ctx.get('continues_topic'):
            lines.append("**Prior Context:**")
            lines.append(f"- Continues topic: {prior_ctx.get('continues_topic')}")
            if prior_ctx.get('prior_turn_purpose'):
                lines.append(f"- Prior turn purpose: {prior_ctx['prior_turn_purpose']}")
            if prior_ctx.get('relationship'):
                lines.append(f"- Relationship: {prior_ctx['relationship']}")
    ...
    def append_section(self, section_num: int, title: str, content: str):
        ...
        Raises:
            ValueError: If section_num is not 1-6 or section already exists
        """
        if section_num < 1 or section_num > 6:
            raise ValueError(f"Section must be 1-6 (§0 is query, set at init). Got: {section_num}")
    ...
    def extend_section(self, section_num: int, additional_content: str):
        ...
        """
        if section_num < 1 or section_num > 6:
            raise ValueError(f"Section must be 1-6. Got: {section_num}")
    ...
    def update_execution_state(
        ...
        Args:
            ...
            phase_name: Human-readable phase name (e.g., "Reflection", "Planner")
    ...
    def inject_available_skills(self, skills_xml: str):
        """
        Inject available skills into the context for Planner visibility.

        The skills XML is stored in metadata and included when building
        context.md so the Planner knows what skills can be invoked.

        Args:
            skills_xml: XML block from SkillRegistry.get_available_skills_xml()
        """
        self.metadata["available_skills"] = skills_xml
    ...
    def get_markdown(self) -> str:
        """
        Return complete markdown document with §0-5.
        ...
        **Session:** {session_id}
        ...
        # Include available skills if present (for Planner visibility)
        if "available_skills" in self.metadata:
            lines.extend([
                "",
                self.metadata["available_skills"],
                "",
            ])
